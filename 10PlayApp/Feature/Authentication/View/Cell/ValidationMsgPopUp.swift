...
class ValidationMsgPopUp: SlidePopUpView {
    ...
    static func show(on viewController: UIViewController, message: String, isSucsess:Bool, iconName:String? = nil) {
        
        guard let popup = Bundle.main.loadNibNamed("ValidationMsgPopUp", owner: nil)?.first as? ValidationMsgPopUp else { return }
        
        if let iconName = iconName {
            popup.configureUI(with: message, iconName: iconName)
        } else {
            popup.configure(with: message, isSuccess: isSucsess)
        }
        
        
        let popupWidth = viewController.view.frame.width * 0.8
        ...
        let finalHeight = max(dynamicSize.height, 180)
        popup.frame = CGRect(x: 0, y: 0, width: popupWidth, height: finalHeight)
...
extension ValidationMsgPopUp {
    ...
    func configureUI(with message: String, iconName:String) {
        errorLabel.text = message
        img.image = UIImage(named: iconName)
    }
